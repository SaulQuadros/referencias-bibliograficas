...
with st.expander("➕ Adicionar nova referência", expanded=False):
    ...
    if st.button("Salvar Referência", key="save_new"):
        entry = dict(zip(COLS, [db, periodico, auth, yr, ttl,
                                qualis, jcr, mtype, summ, res, rel]))
        pd.concat([df, pd.DataFrame([entry])],
                  ignore_index=True).to_csv(CSV_FILE, index=False)
...
min_year, max_year = (int(df['Ano'].min()), int(df['Ano'].max()))     if not df['Ano'].dropna().empty else (2000, 2025)
if min_year > max_year:
    min_year, max_year = max_year, min_year
filtro_ano = (st.slider("Ano mínimo", min_year, max_year, min_year)
              if min_year < max_year
...
if titulo_search:
    mask = filtered['Título do Artigo'].str.contains(titulo_search,
                                                   case=False,
                                                   na=False)
...
if filtered.empty:
    ...
    table_html.append('<table style="border-collapse: collapse; '
                      'width:100%;">')
    # Cabeçalho
    ...
    for col in COLS:
        table_html.append(
            ...
            f'min-width:120px; white-space:nowrap;">{col}</th>')
    table_html.append(
        '<th style="border:1px solid #ccc; padding:6px;">Editar</th>'
        '<th style="border:1px solid #ccc; padding:6px;">Excluir</th>')
    ...
    for i, row in filtered.iterrows():
        ...
        for col in COLS:
            ...
            table_html.append(
                ...
                f'max-width:200px; white-space:nowrap; overflow:hidden; '
                f'text-overflow:ellipsis;">{val}</td>')
        table_html.append(
            f'<td style="border:1px solid #ddd; padding:6px; '
            f'text-align:center;"><a href="?edit_idx={i}">✏️</a></td>')
        table_html.append(
            f'<td style="border:1px solid #ddd; padding:6px; '
            f'text-align:center;"><a href="?del_idx={i}">🗑️</a></td>')
        table_html.append('</tr>')
...
if edit_idx is not None:
    ...
    yr_e        = cols2.number_input("Ano", 1900, 2100, value=int(yr_e))
    ...
    mtype_e     = cols2.selectbox("Tipo de Modelo",
                      ["Empírico","Regressão","ANN","GA","GEP","Outros"],
                      index=["Empírico","Regressão","ANN","GA","GEP","Outros"].index(mtype_e))
    ...
    if col1.button("Confirmar Alteração"):
        df.at[edit_idx, COLS] = [db_e, periodico_e, auth_e, yr_e,
                                 ttl_e, qualis_e, jcr_e, mtype_e,
                                 summ_e, res_e, rel_e]
...
# Exclusão similar ao anterior ...
# Botão de download
csv_data = df.to_csv(index=False).encode('utf-8')
st.download_button("📥 Baixar planilha CSV",
                   data=csv_data,
